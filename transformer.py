...
class DataTransformer:
    ...
    def convert_data_types(self) -> pd.DataFrame:
        """Converts data types of specific columns."""
        self.df["pitch_type"] = self.df["pitch_type"].astype(str)
        self.df["game_date"] = pd.to_datetime(self.df["game_date"], errors='coerce')
        self.df["release_speed"] = pd.to_numeric(self.df["release_speed"], errors='coerce')
        self.df["release_pos_x"] = pd.to_numeric(self.df["release_pos_x"], errors='coerce')
    ...
    def add_features(self) -> pd.DataFrame:
        if "release_speed" in self.df.columns and "release_pos_x" in self.df.columns and "release_pos_z" in self.df.columns:
            self.df["release_magnitude"] = np.sqrt(self.df["release_speed"]**2 + self.df["release_pos_x"]**2 + self.df["release_pos_z"]**2)
   
        if "release_speed" in self.df.columns and "spin_rate_deprecated" in self.df.columns:
            self.df["spin_efficiency"] = self.df["spin_rate_deprecated"] / self.df["release_speed"]
        return self.df

    def transform(self) -> pd.DataFrame:
        """Applies all transformation steps to the DataFrame."""
        self.clean_column_names()
        self.remove_duplicates()
        self.handle_missing_values()
        self.convert_data_types()
